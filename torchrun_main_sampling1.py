...
from galore_torch import GaLoreAdamW_sampling1, GaLoreAdamW8bit, GaLoreAdafactor
...
def parse_args(args):
    ...
    parser.add_argument("--save_dir", type=str, default="/scratch-shared/HTJ/checkpoints/")
    ...
    parser.add_argument("--zero_grad",default=False,action="store_true")
    parser.add_argument("--mask_grad",default=False,action="store_true")
    parser.add_argument("--sampling",default=False,action="store_true")
    parser.add_argument("--zero_all_state",default=False,action="store_true")
...
def main(args):
    ...
    if 'galore' in args.optimizer.lower():
        ...
        param_groups = [{'params': regular_params,'zero_all_state':args.zero_all_state}, 
                        {'params': galore_params,'mask_grad':args.mask_grad, 'rank': args.rank,'zero_grad':args.zero_grad,'sampling':args.sampling,'zero_all_state':args.zero_all_state,'zero_state':args.zero_state, 'update_proj_gap': args.update_proj_gap, 'scale': args.galore_scale, 'proj_type': args.proj_type}]
        if args.mask_grad:
            assert args.zero_grad
            random_init_mask_hook(galore_params,args.rank)
    ...
    if args.optimizer.lower() == "adam":
        ...
        optimizer = GaLoreAdamW_sampling1(param_groups, lr=args.lr, weight_decay=args.weight_decay,updating_mask_method=args.updating_mask_method,warmup_epoch=args.warmup_epoch)
    # implement sgd
