...
#SBATCH --gpus=1
#SBATCH --job-name=zero_state_keep_grad_weight_max
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=18

#SBATCH --time=20:00:00
#SBATCH --output=./logs/updatemask%A_new_ds_0.1_1.0_zero_state_keep_grad_weight_max.out
...
for pair in "${pairs[@]}"; do
    ...
    torchrun --standalone --nproc_per_node 1 torchrun_main_sampling1.py \
        ...
        --proj_type std \
        --updating_mask_method weight_max \
        --zero_state \
        --zero_all_state \
        --warmup_epoch 25 
done
